...
void drawSpriteTex( int x, int y, int width, int height, float* textureCoords )
{
	GX_Begin(GX_QUADS, GX_VTXFMT0, 4);			// Rozpoczęcie rysowania kwadratu
		...
		GX_Position2f32(x+width-1, y);			// Pozycja prawego górnego narożnika kwadratu na scenie
		GX_TexCoord2f32(textureCoords[2],textureCoords[3]); // Pozycja (skala) prawego górnego narożnika pozycji tektury w obrazie
