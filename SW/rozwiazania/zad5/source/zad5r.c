...
void drawSpriteTex( int x, int y, int width, int height, float* textureCoords )
{
	GX_Begin(GX_QUADS, GX_VTXFMT0, 4);			// Rozpoczęcie rysowania kwadratu
		GX_Position2f32(x, y);					// Pozycija lewego górnego narożnika kwadratu na scenie
...
//---------------------------------------------------------------------------------
	...
	// ladowanie mario
	TPLFile spriteTPL_mario;
	TPL_OpenTPLFromMemory(&spriteTPL_mario, (void *)mario_tpl,mario_tpl_size);
	TPL_GetTexture(&spriteTPL_mario,mario,&texObj_mario);
...
//--------------------------------------------------------
		...
		// zmiana wyswietlanego obrazu co 30 klatek
		if (counter <= 0)
		{
			counter = 30;
			id++;
		...
		// ladowanie tekstury do opengl
		GX_LoadTexObj(&texObj_mario, GX_TEXMAP0);
		drawSpriteTex( 100, 100 , 32, 32, texCoords[id]);

		GX_LoadTexObj(&texObj, GX_TEXMAP0);
		drawSpriteTex( 400, 400 , 32, 32, texCoords[id]);
